...
## Q-1. ウィンドウサイズを変数に入れ、アラートを表示させてみましょう
...
ウィンドウの横幅を取得できるということは、高さも取得できるということになります。 
...
<details><summary>解説</summary><div>
実践では今回のような「特定の文字列を取り除く」条件などはよくあります。
分からないことでも調べて実装する力は大切な技術なので、 日頃から挑む力を身につけておきましょう。
</div></details>

## Q-4. 5秒経過後にアラートを表示させてみましょう
...
変数fruitsに配列を代入してください。
 代入する果物は "リンゴ"、"グレープ"、"バナナ"です。代入後、 "グレープ" をアラートに表示させてください。
...
<details><summary>解説</summary><div>
親要素の下の子要素先頭に追加する際にはprepend()を使用します。
</div></details>
...
<details><summary>解説</summary><div>
クリックされる前にはcssでdisplay:none;を当てておき、クリックされたら、show()をつかって表示します。</div></details>
